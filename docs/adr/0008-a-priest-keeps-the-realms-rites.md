...
The **blessing** is the one perk that is simply given, and **mass** is the only way to it. Every seventh in-game day the bell tolls, the celebrant calls mass at the church and speaks the liturgy over the heads of whoever comes; each member at the altar takes a couple of minutes of regeneration and resistance, free, once to a mass. It began as a `/kingdom church bless` command a priest could fire at a name, which made him a potion vending machine on a cooldown — the thing this design was chosen instead of. Tying the blessing to a weekly ceremony people must physically attend gives the church a congregation and a calendar without a faith score behind either.

**Coronation** is the rite with teeth. Titles remain the operator's to assign — that does not change — but a monarch who has not been crowned may not do the ceremonial half of the job: no royal assent, no honours, no swearing of roles or granting of titles. The everyday powers, permits and whitelist and `/kingdom` administration, are untouched, and the gate only bites at all in a kingdom that has a consecrated church to be crowned in. A realm cannot be bricked by having no priest; it can only be left with an uncrowned king who cannot yet sign a bill. The priest crowns the rightful holder and no one else — he consecrates the succession, he does not choose it. The rite is asked for at the cleric rather than typed: right-clicking the church's cleric opens the Coronation window to the King, Queen or a Prince of that realm alone, which is also what keeps the cleric's vanilla trading window shut.
