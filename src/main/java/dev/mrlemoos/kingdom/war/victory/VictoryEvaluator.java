 ...
 * satisfaction — demobilises via the optional {@link DemobilisationService} hook and dispatches
 * the named {@link dev.mrlemoos.kingdom.model.war.WarOutcome} to the optional {@link
 * VictoryOutcomeDispatcher} hook exactly once. Neither hook is required: without a
 * demobilisation service the war domain-state is simply left untouched, and without an outcome
 * dispatcher the outcome notification is simply skipped — victory is still recorded either way.
 *
 ...
 * it only short-circuits straight to victory once the aim is met.
 */
public final class VictoryEvaluator {
    ...
    private void dispatchOutcome(ActiveWar war) {
        if (outcomeDispatcher == null) {
            return;
        }
        switch (war.outcome()) {
            case ANNEXATION -> outcomeDispatcher.onAnnexation(war);
