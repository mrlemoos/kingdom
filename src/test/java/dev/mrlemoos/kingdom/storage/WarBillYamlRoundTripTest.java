...
class WarBillYamlRoundTripTest {

    @Test
    void currentWarBillPayloadRoundTrips() {
        Kingdom kingdom = new Kingdom("northmarch", "Northmarch");
        Bill bill = new Bill(
                "northmarch-4",
                "northmarch",
                BillType.WAR,
                "Declaration of war on Southreach",
                BillState.TABLED,
                UUID.fromString("22222222-2222-2222-2222-222222222222"),
                new BillPayload.War("southreach", WarAim.CAPITAL_FALL, WarOutcome.WAR_TRIBUTE, 5),
                4_000L);
        ...
        BillPayload.War payload = (BillPayload.War) loadedBill.payload();
        assertEquals("southreach", payload.targetKingdomId());
        assertEquals(WarAim.CAPITAL_FALL, payload.aim());
        assertEquals(WarOutcome.WAR_TRIBUTE, payload.outcome());
        assertEquals(5, payload.musterDeadlineMcDays());
        assertEquals(BillType.WAR, loadedBill.type());
    }
}
