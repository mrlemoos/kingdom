...
**Lapsed warrant**:
An **active warrant** nobody served within the statute of limitations: a configured number of **realm days** (default seven) counted from the day it became active—Crown approval, or issue for a **flagrant warrant**. A warrant awaiting **royal warrant approval** never lapses. On lapse the **wanted nametag** clears and any **arrest reward** returns to its poster; the suspect and poster are told privately. The loyalty drop from the original breach stands—the lapse ends the pursuit, not the offence.
_Avoid_: Expired warrant, pardon, amnesty, cancelled warrant
...
**Prison labour**:
Work a player convict does to shorten a **prison sentence**: mining the configured block (stone by default) within eight blocks of their cell. The block is not broken. Each block takes a few seconds off the sentence (five by default), no more than one block a second counts, and labour may never take off more than half the original sentence. Villager convicts do not labour. A later commutation halves whatever time remains.
_Avoid_: Hard labour, community service, work release
